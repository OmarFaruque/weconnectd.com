...
function updateCustomurl($username, $url, $connection) {
        $stmt = $connection->prepare("UPDATE users SET url = ? WHERE username = ?");
        $stmt->bind_param("ss", $css, $username);
        $stmt->execute();
        $stmt->close();
...
/**
 * Summary of updateUserCryptoProfile
 * @param mixed $username
 * @param mixed $profile
 * @param mixed $connection
 * @return void
 */
function updateUserCryptoProfile($username, $profile, $connection)
{
    $stmt = $connection->prepare("UPDATE users SET cryptoportfolio = ? WHERE username = ?");
