...
if(isset($_GET['agent_id'])){
    ...
    $edit = $conn->query("SELECT * FROM `agents_suppliers` WHERE `user_type`='Supplier' AND id='$agent_id'");
    if($edit->num_rows == 1){
...
if (isset($_POST['submit'])) {
    $status = $_POST['status'];
    $save_data_in_db = "UPDATE `agents_suppliers` SET `name`='$name', `email`='$email`, `mobile`='$mobile', `state`='$state', `city`='$city', `address`='$address', `status`='$status' WHERE id='$agent_id'";
  
    $result = $conn->query($save_data_in_db);
    if ($result === TRUE) {
      echo "<script>
      alert('Updated Successfully');
      location.href = 'edit-agents.php?agent_id=$agent_id';
      </script>";
    } else {
      echo "<script>
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Update Agents</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                <div class="card-body">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input type="text" value="<?php echo $edit_d['name']; ?>" class="form-control" id="name">
                    </div>
                  </div> 
                  <div class="form-group row">
                    <label for="title" class="col-sm-2 col-form-label">Email</label>
                    <div class="col-sm-10">
                      <input type="email" value="<?php echo $edit_d['email']; ?>" class="form-control" id="email">
                    </div>
                  </div> 
                  <div class="form-group row">
                    <label for="title" class="col-sm-2 col-form-label">Mobile</label>
                    <div class="col-sm-10">
                      <input type="email" value="<?php echo $edit_d['mobile']; ?>" class="form-control" id="mobile">
                    </div>
                  </div>
                  <div class="form-group row">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input type="email" value="<?php echo $edit_d['mobile']; ?>" class="form-control" id="city">
                    </div>
                  </div> 
                  <div class="form-group row">
