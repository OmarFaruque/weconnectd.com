...
if(isset($_SESSION['siteusername'])){
 if(isset($_POST['reciever']) AND isset($_POST['channel'])){
   ...
   $seen_c = $dbconn->query("SELECT * FROM ghost_chat WHERE `reciever`='$reciever' AND `channel`='$channel' AND `sender`!='$self' AND `status`=0");
	if($seen_c->num_rows > 0){
        ...
        while($blog_d = $seen_c->fetch_assoc()){
			...
			$set_seen = $dbconn->query("UPDATE ghost_chat SET `status`=1 WHERE `chatid`='$chatid'");
        }
        ...
        $result = array(
                "status"=>"error",
                "error"=>"There is more results $reciever"
                );
        echo json_encode($result);
    }
