...
if($auth === TRUE){
  ?>
<?php 
$about = $conn->query("SELECT * FROM `terms_and_conditions` WHERE id=1")->fetch_array();
?>
...
if (isset($_POST['submit'])) {
  ...
  $text = $_POST['text'];
      ...
      $save_data_in_db = "UPDATE `terms_and_conditions` SET `title`='$title', `text`='$text' WHERE id=1;";
  
      $result = $conn->query($save_data_in_db);
      if ($result === TRUE) {
        echo "<script>
        alert('Updated Successfully');
        location.href = 'terms-and-conditions.php';
        </script>";
      } else {
        echo "<script>
              ...
              <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                <div class="card-body">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <?php
                        $m = $conn->query("SELECT * FROM meta_tag WHERE page='terms_and_conditions' AND page_row_id='1'");
                        if($m->num_rows > 0){
                            ?>
                            <a href="edit-meta-tag.php?page=terms_and_conditions&page_row_id=1"><i class="fa fa-edit"></i></a>
                            <?php
                        } else{
                            ?>
                            <a href="add-meta-tag.php?page=terms_and_conditions&page_row_id=1"><i class="fa fa-plus"></i></a>
                            <?php
                        }
                        ?>
                    </div>
