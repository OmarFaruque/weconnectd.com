...
if(isset($_GET['status'])){
  $status = $_GET['status'];
  $d = $conn->query("SELECT * FROM `agents_suppliers` WHERE `user_type`='Agent' AND `status`='$status' ORDER BY `id` DESC");
} else {
  $d = $conn->query("SELECT * FROM `agents_suppliers` WHERE `user_type`='Agent' ORDER BY `id` DESC");
}
?>
<!DOCTYPE html>
<html lang="en">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped table-hover">
                  ...
                  <tr>
                    ...
                    <td><?php echo $data['de_time']."-".$data['ar_time']; ?></td>
                    <td><?php echo $data['company_name']; ?></td>
                    <td><a href="aadhar_card/<?php echo $data['aadhar_card']; ?>">Aadhar Card</a></td>
                    <td><?php echo $data['status']; ?></a></td>
                    <td>
                      <a href="edit-agent.php?agent_id=<?php echo $data['id']; ?>"><i class="fas fa-edit text-warning"></i></a>
