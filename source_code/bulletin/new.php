    ...
    <body>
        <div class="container">
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if(!$_POST['comment']){ $error = "your bulletin body cannot be blank"; goto skipcomment; }
                if(strlen($_POST['comment']) > 10000){ $error = "your bulletin must be shorter than 10000 characters"; goto skipcomment; }
                ...
                if($_POST['allowcomments'] == "true") {
                    $stmt = $conn->prepare("INSERT INTO `bulletin` (author, message, subject) VALUES (?, ?, ?)");
                    ...
                    $stmt = $conn->prepare("INSERT INTO `bulletin` (author, message, subject, comment) VALUES (?, ?, ?, 'n')");
                    $stmt->bind_param("sss", $_SESSION['siteusername'], $text, $subject);
                    $text = htmlspecialchars($_POST['comment']);
                    $subject = htmlspecialchars($_POST['subject']);
                    $stmt->execute();
                    $stmt->close();
                    logDB($_SESSION['siteusername'] . " has made a bulletin named " . $text, $conn);
                }
                skipcomment:
            }
            ?>
            ...
            <div class="padding">
                <span id="padding10">
                    <small>WeConnectd / Bulletin / New</small>
                </span><br>
                <div class="customtopLeft">  
                    <div class="splashBlue">
                        Remember to make sure that your bulletin post is not innapropriate or harrases a person or a group of people! Have fun.
                    </div><br>
                </div>
