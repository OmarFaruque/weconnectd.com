...
function getInfoFromSportsBlog($id, $connection) {
        $stmt = $connection->prepare("SELECT * FROM sports WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
    $result = $stmt->get_result();
    $user = $result->fetch_assoc();
...
function getInfoFromVlog($id, $connection) {
        $stmt = $connection->prepare("SELECT * FROM videos WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
    $result = $stmt->get_result();
    $user = $result->fetch_assoc();
    if($result->num_rows === 0) return('That Vlog does not exist.');
    $stmt->close();

    return $user;
}
function getInfoFromBulletin($id, $connection) {
        $stmt = $connection->prepare("SELECT * FROM bulletin WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
    $result = $stmt->get_result();
    $user = $result->fetch_assoc();
    if($result->num_rows === 0) return('That Bulletin does not exist.');
    $stmt->close();
 ...
 * 
 * @param $name {username}
 * @param $connecton {database connection}
 */
...
function getLikesFromSportsBlog($id, $connection) {
    $stmt = $connection->prepare("SELECT * FROM sports_bloglikes WHERE toid = ? AND type = 'l'");
    $stmt->bind_param("s", $id);
    $stmt->execute();
    $result = $stmt->get_result();
    $rows = mysqli_num_rows($result); 
...
function getDislikesFromSportsBlog($id, $connection) {
    $stmt = $connection->prepare("SELECT * FROM sports_bloglikes WHERE toid = ? AND type = 'd'");
    $stmt->bind_param("s", $id);
    $stmt->execute();
    $result = $stmt->get_result();
    $rows = mysqli_num_rows($result); 
